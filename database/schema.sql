...
-- Hakkımızda sayfası
INSERT INTO pages (slug, title, hero_image, hero_title, hero_subtitle, content, meta_title, meta_description) VALUES
('hakkimizda', 'Hakkımızda', '/images/about-hero.jpg', 'Zarafetiyle Bir Hikaye', 'Han Kuyumculuk',
'<p>Han Kuyumculuk, ışığın içinden doğan bu anlamı taşıyarak her kadının kalbinde özel bir yer bırakır; çünkü her ışıltının ardında unutulmaz bir hikâye vardır.</p><p>Han Kuyumculuk, bir pırlantadan çok daha fazlasını temsil eder; değer verilen kişiyi, paylaşılan özel anları ve kalpten gelen duyguları ışığa dönüştüren bir semboldür.</p>',
'Hakkımızda - Han Kuyumculuk', 'Han Kuyumculuk hikâyesi, değerlerimiz ve vizyonumuz.');
