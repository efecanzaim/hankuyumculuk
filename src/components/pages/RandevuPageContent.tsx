...
interface RandevuPageContentProps {
  locale: Locale;
}

function RandevuContentInner({ locale }: RandevuPageContentProps) {
  ...
  const MONTHS = (dict.appointment as Record<string, unknown>).months as string[];
  const DAYS_FULL = (dict.appointment as Record<string, unknown>).days as string[];
  ...
  // URL'den gelen subject degistiginde formu guncelle
  useEffect(() => {
    if (subjectFromUrl) {
  ...
  // Secilen tarih icin musait saatleri getir (Google Apps Script'ten)
    ...
    try {
      ...
      if (GOOGLE_SCRIPT_URL) {
        ...
        if (data.availableSlots && Array.isArray(data.availableSlots)) {
          // Google Script'ten gelen musait saatler
      ...
      // Google Script yoksa veya hata olduysa, tum saatleri musait goster
      ...
      // Hata durumunda varsayilan slotlari goster (11:00-16:00 calisma saatleri)
      setAvailableSlots(DEFAULT_TIME_SLOTS.map(time => ({
        time,
        available: true
      })));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedDate || !selectedTime) {
      setSubmitError(t('appointment.selectDateTimeError'));
      return;
    }
    ...
    try {
      // Tarih ve saati ISO formatina cevir
      const dateStr = formatDateString(selectedDate);
      const dateTime = `${dateStr}T${selectedTime}:00`;

      if (GOOGLE_SCRIPT_URL) {
        // Google Apps Script'e GET ile gonder (CORS icin)
        ...
        if (data.success) {
          ...
          // Cakisma kontrolu
          if (data.conflict) {
            setSubmitError(t('appointment.slotTaken'));
            // Saatleri yeniden yukle
            fetchAvailableSlots(selectedDate);
          } else {
            setSubmitError(data.message || t('common.errorOccurred'));
          }
        }
      } else {
        // Google Script URL yoksa demo mod
        console.log('Demo mode - Appointment info:', {
          ...formData,
          dateTime,
        });
        setIsSubmitted(true);
                  ...
                  <>
                    <div className="text-center mb-10">
                      ...
                      <p
                        ...
                        style={{ fontFamily: 'var(--font-bw-modelica), sans-serif' }}
                      >
                        {t('appointment.selectDateTimeDesc')}
                      </p>
                    </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                          ...
                          <div>
                            ...
                            <select
                              ...
                              style={{ fontFamily: 'var(--font-bw-modelica), sans-serif' }}
                            >
                              <option value="mucevher">{t('appointment.subjects.jewelry')}</option>
                              <option value="koleksiyon">{t('appointment.subjects.collection')}</option>
                              <option value="size-ozel">{t('appointment.subjects.custom')}</option>
                              <option value="hediye">{t('appointment.subjects.gift')}</option>
                              <option value="erkeklere-ozel">{t('appointment.subjects.men')}</option>
                              <option value="preloved">{t('appointment.subjects.preloved')}</option>
                              <option value="diger">{t('appointment.subjects.other')}</option>
                            </select>
                          </div>
...
export default function RandevuPageContent({ locale }: RandevuPageContentProps) {
  return (
    <Suspense fallback={
      ...
      <RandevuContentInner locale={locale} />
    </Suspense>
  );
}
